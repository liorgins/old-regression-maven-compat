...
public class JSystemEnvControllerOld extends SystemObjectImpl implements
		TestListener {

	private static Logger log = Logger.getLogger(JSystemEnvControllerOld.class
			.getName());
	...
	public JSystemClient getJSystemEnv() throws Exception {
		...
		if (jsystemPropertiesFile.exists()) {
			...
			if (tfolde != null) {
				classPath = tfolde + File.pathSeparator + classPath;
			}
		}
		if (!useExistingServer) {
			...
			(new SystemOutReader(p.getErrorStream(),
					distRunOut.getAbsolutePath() + File.separatorChar
							+ "error.txt")).start();
			JSystemClient jsystem = new JSystemClient(p, foundPort,
					distRunOut.getAbsolutePath());
	...
	public void kill() {
		try {
			...
			report.report(
					"didn't succeed in exiting jsysClient or appClient inside"
	...
	public void extract(String envZipPath) throws Exception {
		...
		String zipPath = JSystemProperties.getCurrentTestsPath()
				+ File.separatorChar + envZipPath;
			...
			if (jarListFileExist) {
				...
				if (runnerSourceDir == null) {
					...
					for (int i = 0; i < JSYSTEM_THIRDPARTY.length; i++) {
						FileUtils.copyFile(
								findInClassPath(JSYSTEM_THIRDPARTY[i]),
					...
					FileUtils.copyDirectory(new File(runnerOutDir
							+ File.separator + "thirdparty/ant"), ant);
				} else {
					...
					for (int i = 0; i < JSYSTEM_THIRDPARTY.length; i++) {
						FileUtils.copyFile(new File(runnerSourceDir
								+ "thirdparty" + File.separator + "commonLib"
								+ File.separator + JSYSTEM_THIRDPARTY[i]),
								new File(thirdpartyCommonLib,
										JSYSTEM_THIRDPARTY[i]));
					}
					for (int i = 0; i < JSYSTEM_JARS.length; i++) {
					...
					FileUtils.copyDirectory(new File(runnerOutDir
							+ File.separator + "thirdparty" + File.separator
							+ "ant"), ant);
	...
	public void setRunnerEnvTestFolder(String regressionBaseTestsFolderParam,
			String sutFileName) throws Exception {
		...
		FileUtils.copyDirectory(new File(regressionBaseTestsFolder),
				testProject);
	...
	private String buildClassPath() {
		...
		buf.append(runnerOutDir + "classes" + File.pathSeparatorChar);
