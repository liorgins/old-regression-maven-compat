...
public class JSystemEnvController extends SystemObjectImpl {
	private static Logger log = Logger.getLogger(JSystemEnvController.class
			.getName());
	...
	public void init() throws Exception {
		...
		if (System.getProperty("user.dir").contains("jregression")) {
			...
			regressionPath = ps.getProperty(FrameworkOptions.TESTS_CLASS_FOLDER
					.toString());
			regressionPath = regressionPath.substring(0,
					regressionPath.length() - 7); // if value is set, cut off
													// the classes
	...
	public void buildRunnerEnv() throws Exception {
		...
		if (jarListFileExist) {
			...
			Properties p = FileUtils
					.loadPropertiesFromFile(runnerOutJarListProperties
							.getAbsolutePath());
			...
			// if in sut there is no runner source dir mentioned, then copy from
			// classpath
			...
			if (runnerSourceDir == null) {
				...
				for (int i = 0; i < JSYSTEM_THIRDPARTY.length; i++) {
					FileUtils
							.copyFile(findInClassPath(JSYSTEM_THIRDPARTY[i]),
									new File(thirdpartyCommonLib,
											JSYSTEM_THIRDPARTY[i]));
				}
				for (int i = 0; i < JSYSTEM_JARS.length; i++) {
					FileUtils.copyFile(findInClassPath(JSYSTEM_JARS[i]),
							new File(outLib, JSYSTEM_JARS[i]));
				}
				FileUtils.copyDirectory(new File(runnerOutDir + File.separator
						+ "thirdparty" + File.separator + "ant"), ant);

				// if there is a source dir defined in sut, copy from the source
				// dir, to the
				// correct location.
			} else {
				...
				File xmlrpcJarTemp = new File(runnerSourceDir + "lib"
						+ File.separator + "xmlrpc.jar");
				FileUtils.copyFile(findInClassPath("xmlrpc.jar"),
						xmlrpcJarTemp);

				File jemmyJarTemp = new File(runnerSourceDir + "lib"
						+ File.separator + "jemmy.jar");
				FileUtils.copyFile(findInClassPath("jemmy.jar"),
						jemmyJarTemp);

				for (int i = 0; i < JSYSTEM_THIRDPARTY.length; i++) {
					FileUtils.copyFile(new File(runnerSourceDir + "thirdparty"
							+ File.separator + "commonLib" + File.separator
							+ JSYSTEM_THIRDPARTY[i]), new File(
							thirdpartyCommonLib, JSYSTEM_THIRDPARTY[i]));
				}
				for (int i = 0; i < JSYSTEM_JARS.length; i++) {
					FileUtils.copyFile(new File(runnerSourceDir + "lib"
							+ File.separator + JSYSTEM_JARS[i]), new File(
							outLib, JSYSTEM_JARS[i]));
				}
				FileUtils.copyDirectory(new File(runnerOutDir
						+ File.separatorChar + "thirdparty"
						+ File.separatorChar + "ant"), ant);
				FileUtils.deleteFile(regressionJarTemp.getAbsolutePath());
				FileUtils.deleteFile(xmlrpcJarTemp.getAbsolutePath());
				FileUtils.deleteFile(jemmyJarTemp.getAbsolutePath());
			}
		}
		...
		// and copy the jregression jar from jregression/lib
			...
			FileUtils.copyDirectory(runnerOutDir + "lib",
					distRunOut.getAbsolutePath() + File.separatorChar
							+ "lib");
			FileUtils.copyDirectory(runnerOutDir + "thirdparty",
					distRunOut.getAbsolutePath() + File.separatorChar
							+ "thirdparty");
			FileUtils.copyFile(regressionPath + File.separatorChar + "lib"
					+ File.separatorChar + "jregression.jar",
					distRunOut.getAbsolutePath() + File.separatorChar
							+ "thirdparty" + File.separatorChar
							+ "jregression.jar");
			FileUtils.copyFile(regressionPath + File.separatorChar + "lib"
					+ File.separatorChar + "xmlrpc.jar",
					distRunOut.getAbsolutePath() + File.separatorChar
							+ "thirdparty" + File.separatorChar
							+ "xmlrpc.jar");
			FileUtils.copyFile(regressionPath + File.separatorChar + "lib"
					+ File.separatorChar + "jemmy.jar",
					distRunOut.getAbsolutePath() + File.separatorChar
							+ "thirdparty" + File.separatorChar
							+ "jemmy.jar");
		}
	...
	public void setInitialJsystemProperties() throws Exception {
		for (Entry<FrameworkOptions, String> entry : Commons
				.getBaseJsystemProperties().entrySet()) {
			JSystemProperties.getInstance().setPreference(entry.getKey(),
					entry.getValue());
		}
	}

	/**
	 * create and run the JServer process that runs an xmlrpc server to listen
	 * on requests. can be opened with debug option for remote or not.
	 * 
	 * @throws Exception
	 */
		...
		if (p == null) {
			...
			if (!StringUtils.isEmpty(debugRemoteRunnerPort)) {
				...
				command = new String[] {
						...
						"-Xnoagent",
						"-Djava.compiler=NONE",
						"-Xrunjdwp:transport=dt_socket,address=${" + values[0]
								+ "},server=y,suspend=" + values[1],
						"-classpath", classPath, JServer.class.getName(),
						Integer.toString(foundPort) };
			} else {
				command = new String[] { "java", "-classpath", classPath,
						JServer.class.getName(), Integer.toString(foundPort) };
			}
			...
			(new SystemOutReader(p.getInputStream(),
					distRunOut.getAbsolutePath() + File.separatorChar
							+ "out.txt")).start();
			(new SystemOutReader(p.getErrorStream(),
					distRunOut.getAbsolutePath() + File.separatorChar
							+ "error.txt")).start();
	...
	/**
	 * creates a client that is compatible with the argument passed for handler
	 * type. sets it's run attributes so it will talk to the server. and returns
	 * the instance.
	 * 
	 * @param handler
	 * @return
	 ...
	 */
	public BaseClient getSystemClient(JServerHandlers handler)
			throws InstantiationException, IllegalAccessException {
		BaseClient clientClass = (BaseClient) handler.getClientClass()
				.newInstance();
	...
	private String buildClassPath() {
		...
		for (int i = 0; i < allJars.size(); i++) {
			...
			if (allJars.elementAt(i).getAbsolutePath()
					.contains("runner" + File.separator + "lib")) {
				buf.append("lib");
			}
			if (allJars
					.elementAt(i)
					.getAbsolutePath()
					.contains(
							"runner" + File.separator + "thirdparty"
									+ File.separator + "commonLib")) {
			...
			if (allJars
					...
					.contains(
							"runner" + File.separator + "thirdparty"
									+ File.separator + "ant" + File.separator
									+ "lib")) {
	...
	public void setRunnerEnvTestFolder(String folder, String sutFileName)
			throws Exception {
		...
		if (!StringUtils.isEmpty(debugRemoteTestPort)) { // if debug remote test
			...
			p.setProperty(
					FrameworkOptions.TEST_VM_PARMS.toString(),
		...
		FileUtils.savePropertiesToFile(p,
				jsystemPropertiesFile.getAbsolutePath());
	}
	...
	public void kill() throws Exception {
		 ...
		 */
		XmlHandler.getInstance().handleXmlCommand(
				"close remote JServer",
				JServerHandlers.APPLICATION.getHandlerClassName() + "."
						+ "exit", new Vector<Object>());
		Thread t = new Thread() {
			public void run() {
				if (p != null) {
					try {
